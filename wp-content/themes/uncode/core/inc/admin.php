...
if ( ! function_exists( 'uncode_error_if_uncode_core_is_not_active' ) ) :
	/**
	 * Add admin notice when Uncode Core is not active
	 */
	function uncode_error_if_uncode_core_is_not_active() {

		if ( class_exists( 'UncodeCore_Plugin' ) ) {
			return;
		}

		echo '<div class="error"><p><strong>' . esc_html__( 'Warning: Uncode Core plugin is not active.', 'uncode' ) . '</strong></p><p>' . sprintf( __( 'This theme requires requires <strong>Uncode Core</strong>, <strong><a href="%s">please activate it</a></strong>.', 'uncode' ), admin_url( 'admin.php?page=uncode-plugins' ) ) . '</p></div>';
	}
endif;
add_action( 'admin_notices', 'uncode_error_if_uncode_core_is_not_active' );
