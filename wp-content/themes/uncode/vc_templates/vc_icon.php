...
if ($icon_image === '') {
	...
	if ( $icon_automatic !== '' ) {
		...
		$output_icon .= '<img'. (($position === 'right' || $position === 'left') ? ' style="max-width:none;"' : '').' src="' . esc_attr( $media_code['code'] ) .'" width="' . esc_attr( $media_code['width'] ) .'" height="' . esc_attr( $media_code['height'] ) .'" alt="' . esc_attr( $media_alt ) . '" />';
	} else {
		$output_icon .= $media_code['code'];
		if ($position === 'right' || $position === 'left') {
			if (strpos($media_code['code'], 'style="width:100%"') !== false) {
