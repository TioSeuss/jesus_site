...
if ($shadow === 'yes') {
	$shape .= ' tmb-shadow';
	$tmb_shape .= ' tmb-media-shadowed';
	if ( $shadow_weight === '' )
		$shadow_weight = 'xs';
	if ( $shadow_darker !== '' )
		$shadow_weight = 'darker-' . $shadow_weight;
	$tmb_shape .= ' tmb-media-shadowed-' . $shadow_weight;
}
...
if ($css_animation !== '') {
	...
	if ($animation_delay !== '') $div_data .= ' data-delay="' . esc_attr( $animation_delay ) . '"';
	if ($animation_speed !== '') $div_data .= ' data-speed="' . esc_attr( $animation_speed ) . '"';
}
...
if ($advanced === 'yes') {
	...
	if (isset($layout['icon'][0]) && $layout['icon'][0] !== '') $block_data['icon_size'] = ' t-icon-size-' . $layout['icon'][0];
